...
public class Instancia {
    ...
    public static Object newInstancia(Class clase) throws Exception {
        ...
        for (Method metodo : clase.getDeclaredMethods()) {
            if (metodo.getAnnotations() != null) {
                ...
                if (metodo.getAnnotation(InvocacionMultiple.class) != null) {
                    InvocacionMultiple anotacion = metodo.getAnnotation(InvocacionMultiple.class);

                    for (int i = 0; i < anotacion.vecesInvocar() - 1; i++) {
        ...
        Class<?> cls = Class.forName(clase.getCanonicalName() + "Proxy", true, classLoader);

        f.delete();
        new File(src, clase.getCanonicalName().replaceAll("\\.", "/") + "Proxy.class").delete();
